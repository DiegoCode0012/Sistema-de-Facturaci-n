...
public class FacturaPdfView extends AbstractPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document document, PdfWriter writer,
		...
		//Otra forma para traducir trabaja con el locale y el messagesource por debajo
		MessageSourceAccessor mensajes= getMessageSourceAccessor();
		
		Factura factura =(Factura) model.get("factura"); // este get nos devuelve el objeto factura que pasamos a la vista en el controller
		// de los 2 parametros model.add () se pasa el primero
		...
		tabla2.addCell(cell);
		tabla2.addCell(mensajes.getMessage("text.cliente.factura.folio") + ": " +factura.getDescripcion());
		tabla2.addCell(mensajes.getMessage("text.cliente.factura.fecha") + ": " + factura.getCliente().getCreateAt());
		document.add(tabla);
		document.add(tabla2);
